 ...
 *
 * The optional `transaction` field matches better-sqlite3’s helper; this driver
 * uses explicit `BEGIN IMMEDIATE` / `COMMIT` via `exec` so it stays compatible
 * with async {@link SessionStorageDriver.transaction} callbacks.
 */
...
/**
 * SQLite session storage using `better-sqlite3` (Node.js).
 *
 * ```ts
 ...
 * ```
 */
export class BetterSqlite3Storage implements SessionStorageDriver, SessionStorageTxn {
  readonly #stmts: SessionSqliteStatements;
  #transactionQueue: Promise<void> = Promise.resolve();

  constructor(private readonly db: BetterSqlite3Database) {
    db.exec("PRAGMA foreign_keys = ON");
    db.exec(SESSION_SQLITE_SCHEMA);
